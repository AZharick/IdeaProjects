...
public class MyTelegramBot extends TelegramLongPollingBot {
   ...
   public void onUpdateReceived(Update update) {
      if (update.hasMessage()) {      //если есть изменения и они содержат сообщение
         chat_id = update.getMessage().getChatId();    //иниц-ся перем-я chat_id
         switch (update.getMessage().getText()) {
            ...
            case "/give":
               try {
                  sendMessage(Utils.getUrl(URI));    //высылается ссылка на пикчу
               } catch (IOException e) {
